...
void ConsolaInteractiva::segmentar(const vector<string>& args) {
    ...
    if (args.size() < 4 || (args.size() - 1) % 3 != 0 || args.size() > 16) {
        cout << "Error: Uso incorrecto. " << descripcion_comando("segmentar", true) << "\n";
        cout << "Uso: segmentar salida.pgm x1 y1 l1 [x2 y2 l2 ...]\n";
        return;
    }

    std::string archivo_salida = args[0];
    ...
    for (size_t i = 1; i + 2 < args.size(); i += 3) {
        try {
            ...
            int label = std::stoi(args[i + 2]);
            semillas.push_back({x, y, label});
        } catch (...) {
            cout << "Error: Coordenadas o etiquetas inválidas.\n";
            return;
        }
    }

    Imagen salida_segmentada;
    if (!segmentar_imagen_dijkstra(imagen_actual, semillas, salida_segmentada)) {
        cout << "Error: La segmentación falló.\n";
        return;
    }

    if (guardar_pgm(archivo_salida, salida_segmentada.datos)) {
        cout << "La imagen ha sido segmentada correctamente y almacenada en '" << archivo_salida << "'.\n";
    } else {
        cout << "Error: No se pudo guardar la imagen segmentada como '" << archivo_salida << "'.\n";
    }
}
...
std::string ConsolaInteractiva::descripcion_comando(const std::string& comando, bool detallado) {
    ...
    if (comando == "segmentar") {
        return detallado ? "Uso: segmentar Uso: segmentar imagen.pgm x1 y1 etiqueta1 Segmenta la imagen cargada usando semillas";
    }
